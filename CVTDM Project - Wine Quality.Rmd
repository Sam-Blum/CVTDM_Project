...
Now, let's create 400 new instances for 'Low' quality, and let's add them to the training set:
```{r, warning = FALSE}
set.seed(1)
...
From the plots above, it seems that the best number of neighbours (i.e., number of k's) is k=2 if we want to maximize the balanced accuracy and the kappa value. 

Choosing this parameter(k=2), we can assess the predictive performance of the kNN model on the test set:
```{r, warning = FALSE}
#perform knn classification on the test set using best k = 2
set.seed(1)
best_knn_pred = knn3Train(train = train_norm_df[,-11], test = test_norm_df[,-11], cl = train_norm_df[,11], k = 2, prob = T)
...
### Determination of the best Neural Net with a threshold of 0.1
...
### Determination of the best Neural Net with a threshold of 0.2

```{r}
nn_nodes_2 <- data.frame("number_nodes" = seq(from= 2, to = 10), "kappa_value" = NA, "balanced_accuracy" = NA)
...
Best Neural Net is with threshold = 0.1 and 7 nodes:
```{r}
set.seed(1)
...
ensemble_prob_low_df$mean_prob_low = apply(ensemble_prob_low_df, 1, mean)
...
ensemble_prob_inter_df$mean_prob_inter = apply(ensemble_prob_inter_df, 1, mean)
...
ensemble_prob_high_df$mean_prob_high = apply(ensemble_prob_high_df, 1, mean)
...
ensembles_pred_df$majority_vote_pred = as.factor(apply(ensembles_pred_df[2:9], 1, function(x) names(which.max(table(x)))))
...
```{r, warning = FALSE}
results_df = data.frame("method" = c("knn", "logistic regression", "naive Bayes", "classification tree", "boosted tree", "bagged tree", "random forest", "neural net", "majority vote", "mean probability"),
                        "kappa" = c(kappa_best_knn, kappa_logist, kappa_bayes, kappa_pruned_tree1, kappa_boost1, kappa_bag1, kappa_rf1, kappa_nn1, kappa_majority, kappa_mean_prob),
                        "balanced_accuracy" = c(bal_acc_best_knn, bal_acc_logist, bal_acc_bayes, bal_acc_pruned_tree1, bal_acc_boost1, bal_acc_bag1, bal_acc_rf1, bal_acc_nn1, bal_acc_majority, bal_acc_mean_prob))
head(results_df)
```
