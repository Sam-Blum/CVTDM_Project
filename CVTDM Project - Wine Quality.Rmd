...
(pruned_tree_cm$byClass[3,1] + pruned_tree_cm$byClass[2,1] + pruned_tree_cm$byClass[1,1] ) / 3
```

(2) Boosted tree:
```{r}
set.seed(1)
boost <- boosting(binned_quality ~ ., data=train_df, cp = tree1$cptable[which.min(tree1$cptable[, "xerror"]), "CP"])
...
(boost.cm$byClass[3,1] + boost.cm$byClass[2,1] + boost.cm$byClass[1,1]) / 3
```
...
### Neural Nets
```{r}
nn1 <- neuralnet(binned_quality ~ ., data = train_norm_df, hidden = 1, linear.output = FALSE)
nn1$weights
plot(nn1, rep="best", cex=0.8)

valid_pred_nn1 <- predict(nn1, valid_norm_df[, -11])
```
